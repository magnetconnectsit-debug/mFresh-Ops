...
class ThreeInchReceipt {
  static Future<List<int>> generateEscPosBytes(
    ...
    final normalHeight = PosTextSize.size2;
    final titleSize = PosTextSize.size3;
    ...
    bytes += [27, 32, 2];
    bytes += generator.text("TOTAL: RS. ${service.price}", styles: PosStyles(bold: true, align: align, height: titleSize, width: PosTextSize.size1));
    bytes += [27, 32, 0];
    bytes += generator.text(separator, styles: PosStyles(align: centerAlign));
              ...
              return pw.Padding(
                ...
                child: pw.Column(
                  ...
                  children: [
                    pw.Center(
                      child: pw.Column(
                        children: [
                          ...
                          pw.Text("mFresh", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 24 * scale, letterSpacing: 2.0))
                        ]
                      )
                    ),
                    pw.SizedBox(height: 12),
                    pw.Text("UNIT NO.: ${booking.unitNo}", textAlign: pw.TextAlign.center, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 14 * scale, letterSpacing: 1.0)),
                    pw.Text("Location: ${booking.fullAddress}", textAlign: pw.TextAlign.center, style: pw.TextStyle(fontSize: 11 * scale, fontWeight: pw.FontWeight.normal)),
                    pw.Text(pdfSeparator, textAlign: pw.TextAlign.center, style: pw.TextStyle(fontSize: 10 * scale)),
                    
                    pw.Text("BOOKING ID: ${booking.bookingId}", textAlign: pw.TextAlign.center, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 12 * scale)),
                    pw.Text("Date & Time: ${booking.bookingTimeDate.isNotEmpty ? booking.bookingTimeDate : DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now())}", textAlign: pw.TextAlign.center, style: pw.TextStyle(fontSize: 11 * scale)),
                    
                    pw.Text("Payment: ${booking.paymentMode == 1 ? 'CASH' : booking.paymentMode == 2 ? 'UPI' : 'QR'}", textAlign: pw.TextAlign.center, style: pw.TextStyle(fontSize: 11 * scale)),
                    pw.Text(pdfSeparator, textAlign: pw.TextAlign.center, style: pw.TextStyle(fontSize: 10 * scale)),

                    pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.center,
                      children: [
                        pw.Text(service.servicesName.toUpperCase(), textAlign: pw.TextAlign.center, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 12 * scale)),
                        pw.Text("QTY: 1", textAlign: pw.TextAlign.center, style: pw.TextStyle(fontSize: 11 * scale)),
                        pw.SizedBox(height: 6),
                      ]
                    ),

                    pw.Text(pdfSeparator, textAlign: pw.TextAlign.center, style: pw.TextStyle(fontSize: 10 * scale)),
                    pw.Text("TOTAL: RS. ${service.price}", textAlign: pw.TextAlign.center, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 18 * scale, letterSpacing: 1.5)),
                    pw.Text(pdfSeparator, textAlign: pw.TextAlign.center, style: pw.TextStyle(fontSize: 10 * scale)),
                    ...
                    pw.SizedBox(height: 12),
                    pw.Center(
                      child: pw.Text("Thank you for using mFresh!", style: pw.TextStyle(fontSize: 10 * scale, fontWeight: pw.FontWeight.bold)),
                    ),
