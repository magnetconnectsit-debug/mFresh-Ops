...
import 'package:mfresh_ops/modules/booking/controllers/booking_unit_selection_controller.dart';
...
class ServiceDetailsController extends GetxController {
  ...
  void onInit() {
    ...
    if (user != null) {
      ...
      final canToggle = user.appPermissions.onlineOfflineToggle;
      if (!canToggle) {
        isOnline.value = true;
      }
    }
  ...
  Future<void> refreshData() async {
    await Future.wait([
      ...
      if (Get.isRegistered<BookingUnitSelectionController>()) Get.find<BookingUnitSelectionController>().fetchUnits(),
    ]);

    try {
      if (Get.isRegistered<BookingUnitSelectionController>()) {
         final dashUnits = Get.find<BookingUnitSelectionController>().allUnitsList;
         ...
         final units = await _bookingRepository.getAllUnits();
  ...
  Future<void> fetchServices() async {
    ...
    try {
      ...
      final fetchedServices = await _bookingRepository.getServices(
  ...
  Future<void> verifyMemberPhone(String phone) async {
    ...
    try {
      ...
      final isValid = await _bookingRepository.validateMemberPhone(phone: phone);
      if (isValid) {
        final sent = await _bookingRepository.sendMemberOtp(phone: phone);
  ...
  Future<bool> verifyMemberOtp({
    ...
    try {
      ...
      final verified = await _bookingRepository.verifyMemberOtp(
  ...
  Future<void> initiateBooking({required bool isExternalQr}) async {
    ...
    if (!isCustomer.value) {
      ...
      try {
        isVerifyingMember.value = true;
        final isValid = await _bookingRepository.validateMemberPhone(
          phone: memberPhone,
        );
        if (!isValid) {
          AppCommonToastMessage.show(
        ...
        final sent = await _bookingRepository.sendMemberOtp(phone: memberPhone);
        isOtpSent.value = sent;
        if (sent) {
    ...
    try {
      ...
      final response = await _bookingRepository.initiateBooking(data: body);
  ...
  Future<void> _confirmSuccess(
    ...
    final confirmed = await _bookingRepository.confirmSuccessBooking(
      data: successData,
    );
        ...
        if (status == 'SUCCESS' || status == 'PENDING') {
          ...
          if (phonePeStatus != null) {
            ...
            if (phonePeStatus['success'] == true && code == 'PAYMENT_SUCCESS') {
              ...
              final bookingDetails = await _bookingRepository.getBookingDetails(
                bookingId: bookingId,
              );
